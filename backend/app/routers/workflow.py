...
def list_workflow_nodes(repo: Repository = Depends(get_repository)):
    return repo.list_workflow_nodes()


@router.get("/edges", response_model=list[WorkflowEdgeDto], response_model_exclude_none=True)
def list_workflow_edges(repo: Repository = Depends(get_repository)):
...
def update_workflow_node(node_id: str, request: UpdateWorkflowNodeRequestDto, repo: Repository = Depends(get_repository)):
    return repo.update_workflow_node(node_id, request)


@router.post("/validate", response_model=WorkflowValidationResultDto)
def validate_workflow(repo: Repository = Depends(get_repository)):
    return repo.validate_workflow()


@router.post("/simulate", response_model=WorkflowSimulationResultDto)
def simulate_workflow(repo: Repository = Depends(get_repository)):
    return repo.simulate_workflow()


@router.post("/publish", response_model=PublishWorkflowResultDto)
def publish_workflow(repo: Repository = Depends(get_repository)):
