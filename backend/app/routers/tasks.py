...
def list_tasks(repo: InMemoryRepository = Depends(get_repository)):
    return repo.list_tasks()


@router.post("", response_model=TaskDto, status_code=status.HTTP_201_CREATED, response_model_exclude_none=True)
def create_task(request: CreateTaskRequestDto, repo: InMemoryRepository = Depends(get_repository)):
    return repo.create_task(request)


@router.patch("/{task_id}", response_model=TaskDto, response_model_exclude_none=True)
def update_task(task_id: str, request: UpdateTaskRequestDto, repo: InMemoryRepository = Depends(get_repository)):
    return repo.update_task(task_id, request)


@router.post("/{task_id}/move", response_model=TaskDto, response_model_exclude_none=True)
def move_task(task_id: str, request: MoveTaskRequestDto, repo: InMemoryRepository = Depends(get_repository)):
